...
## 1. 선택 텍스트 → 상세 질문 (포그라운드)

- chatgpt.com이든 아니든 어떤 페이지에서나 텍스트 드래그 → 우클릭 → **"ChatGPT로 상세 질문하기"**
- 또는 선택 후 `Ctrl+Shift+Q` (Mac: `Cmd+Shift+Q`)
- 우클릭 좌표 바로 아래에 floating 패널이 뜨면서 별도 ChatGPT 세션으로 자동 전송됩니다.
- 패널은 헤더 드래그로 이동, 우/하단/우하단 코너로 크기 조절 (마지막 크기는 `localStorage`에 저장).
